...
summary(model)
```
All variables still remain significant in the model and R-squared is a bit higher as well. Updated linear regression model now becomes: ***obesity = β<sub>0</sub> + \beta_1fruit + \beta_2vegetable***. Following, variance inflation factor is calculated to ensure multicollinearity is not present in the equation.
